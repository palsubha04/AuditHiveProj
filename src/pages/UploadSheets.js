...
// import { faUpload, faCalendarAlt, faDownload } from '@fortawesome/free-solid-svg-icons';
...
//       try {
//         const response = await api.get(`/tax/audit-history/latest?tax_type=${formData.type}`);
//         setAuditHistory(response.data);
//       } catch (error) {
//         console.error('Error fetching audit history:', error);
//         if (error.response?.status === 404) {
//           setAuditHistory({ message: `No ${formData.type} data upload found.` });
//         }
//       }
//     };
...
//         } else if (status.status === 'processing' || status.status === 'started') {
...
//       formPayload.append('start_date', formData.startDate.toLocaleDateString('en-GB'));
//       formPayload.append('end_date', formData.endDate.toLocaleDateString('en-GB'));
...
//                 {auditHistory ? (
//                   auditHistory.message === `No ${formData.type} data upload found.` ? (
//                     <div className="no-data-message">
//                       No {formData.type.toUpperCase()} data has been uploaded yet.
//                     </div>
//                   ) : (
//                     auditHistory.message.replace(/gst/i, formData.type.toUpperCase())
//                   )
//                 ) : (
...
//                 {auditHistory && auditHistory.message !== `No ${formData.type} data upload found.` && (
//                   <span className="date-time">
//                     <span>Date: {new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
//                     <span>Time: {new Date().toLocaleTimeString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true })}</span>
//                   </span>
//                 )}
//               </div>
//             </div>
...
//                     { header: 'Tax Account No', accessorKey: 'tax_account_number' },
...
//                   {selectedFile ? (
//                     <>
//                       <p className="upload-text">Selected file: {selectedFile.name}</p>
//                       <span className="change-file" onClick={(e) => { e.stopPropagation(); handleChangeFile(); }}>
//                         Change file
//                       </span>
//                     </>
//                   ) : (
                  ...
                  <div className="date-range-container">
                    <div className="datepicker-container">
                      <DatePicker
                        ...
                        renderCustomHeader={({ date, changeYear, changeMonth, years = [], months = [] }) => {
